// Biblioteca ConsolaUtils para TP de POO
...
HANDLE ConsolaUtils::hconsola = GetStdHandle(STD_OUTPUT_HANDLE);
HANDLE ConsolaUtils::hStdin = GetStdHandle(STD_INPUT_HANDLE);;
HWND ConsolaUtils::hwnd = GetConsoleWindow();
...
void ConsolaUtils::gotoxy(int x, int y) {
	COORD coord;
	coord.X = x;
	coord.Y = y;
...
void ConsolaUtils::setTextColor(WORD color) {
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	GetConsoleScreenBufferInfo(hconsola, &csbi);
...
void ConsolaUtils::setBackgroundColor(WORD color) {
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	GetConsoleScreenBufferInfo(hconsola, &csbi);
	WORD cor = csbi.wAttributes;
	cor &= 0xFF0F;
...
void ConsolaUtils::setScreenSize(int nLinhas, int nCols) {
	COORD tam;
...
// ver notas no .h
void ConsolaUtils::setTextSize(int x, int y) {
#ifdef _MSC_VER
...
void ConsolaUtils::setTextSizeXP(int x, int y){
#ifdef _MSC_VER
...
void ConsolaUtils::drawLine(int x1, int y1, int x2, int y2, int cor){
#ifdef _MSC_VER
	HDC DrawHDC;
...
void ConsolaUtils::drawCircle(int X, int Y, int R, int Pen, int Fill){
#ifdef _MSC_VER
	HDC DrawHDC;
